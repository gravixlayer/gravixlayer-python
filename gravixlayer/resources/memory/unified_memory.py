...
class UnifiedMemory:
    ...
    def switch_configuration(self, embedding_model=None, inference_model=None, index_name=None, cloud_config=None):
        """
    ...
    async def add(
        ...
        vector_result = await vectors.upsert_text(
            text=content, model=active_embedding_model, id=memory_id, metadata=memory_metadata
        )
        ...
        try:
            ...
            for hit in search_results.hits:
                ...
                if min_relevance <= 0.0 or not query.strip() or hit.score >= min_relevance:
                    ...
                    memory_results.append(MemorySearchResult(memory=memory_entry, relevance_score=hit.score))
        ...
        try:
            ...
            if content:
                ...
                await vectors.upsert_text(
                    text=content, model=active_embedding_model, id=memory_id, metadata=updated_metadata
                )
    ...
    async def list_all_memories(
        ...
        if sort_by == "created_at":
            memories = (
                [result.memory for result in memories] if memories and hasattr(memories[0], "memory") else memories
            )
    ...
    async def get_stats(self, user_id: str) -> MemoryStats:
        ...
        try:
            ...
            stats = {"total": 0, "factual": 0, "episodic": 0, "working": 0, "semantic": 0, "last_updated": datetime.min}
    ...
    async def _increment_access_count(self, vectors, memory_id: str):
        ...
        try:
            ...
            logger.debug(f"Failed to update access count for memory {memory_id}: {e}")
    ...
    async def list_available_indexes(self) -> List[str]:
        ...
        try:
            ...
            for idx in index_list.indexes:
                ...
                if idx.metadata and idx.metadata.get("type") == "unified_memory_store":
