...
class LegacyMemoryCompatibility:
    ...
    async def add(
        ...
        if isinstance(content, list):
            return await self.gravix_memory.process_conversation(
                messages=content, user_id=user_id, metadata=metadata, use_inference=infer
            )
        ...
        return await self.gravix_memory.store_memory(
            content=content, user_id=user_id, memory_type=memory_type, metadata=metadata, memory_id=memory_id
        )
    ...
    async def search(
        ...
        results = await self.gravix_memory.find_memories(
            query=query, user_id=user_id, memory_types=memory_types, max_results=top_k, min_relevance=min_relevance
        )
    ...
    async def get_memories_by_type(self, user_id: str, memory_type: MemoryType, limit: int = 50) -> List[MemoryEntry]:
        """
        ...
        """
        return await self.gravix_memory.list_memories(user_id=user_id, memory_type=memory_type, limit=limit)
    ...
    async def get_all_user_memories(self, user_id: str, limit: int = 100) -> List[MemoryEntry]:
        ...
        """
        return await self.gravix_memory.list_memories(user_id=user_id, limit=limit)
    ...
    async def cleanup_working_memory(self, user_id: str) -> int:
        ...
        """
        return await self.gravix_memory.cleanup_expired_memories(user_id=user_id, memory_type=MemoryType.WORKING)
...
class ExternalCompatibilityLayer:
                    ...
                    for query in generic_queries:
                        try:
                            results = await self.unified_memory.search(
                                query=query, user_id=user_id, top_k=limit, min_relevance=0.0, index_name=index_name
                            )
    ...
    async def update(self, memory_id: str, user_id: str, data: str, index_name: Optional[str] = None) -> Dict[str, str]:
        ...
        updated_memory = await self.unified_memory.update(
            memory_id=memory_id, user_id=user_id, content=data, index_name=index_name
        )
