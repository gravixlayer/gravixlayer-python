...
class SyncMemory:
    ...
    def add(
        ...
        vectors.upsert_text(text=content, model=self.embedding_model, id=memory_id, metadata=memory_metadata)
    ...
    def update(
        ...
        try:
            ...
            if content:
                ...
                vectors.upsert_text(text=content, model=self.embedding_model, id=memory_id, metadata=updated_metadata)
    ...
    def get_memories_by_type(self, user_id: str, memory_type: MemoryType, limit: int = 50) -> List[MemoryEntry]:
        ...
        try:
            ...
            for vector_id, vector in all_vectors.vectors.items():
                if (
                    vector.metadata.get("user_id") == user_id
                    and vector.metadata.get("memory_type") == memory_type.value
                ):
    ...
    def _add_raw_messages(
        self, messages: List[Dict[str, str]], user_id: str, metadata: Optional[Dict[str, Any]] = None
    ) -> List[MemoryEntry]:
        """
