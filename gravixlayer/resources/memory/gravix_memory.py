...
class GravixMemory:
    ...
    def _create_memory_metadata(
        self, memory_type: MemoryType, user_id: str, custom_metadata: Optional[Dict[str, Any]] = None
    ) -> Dict[str, Any]:
    ...
    async def store_memory(
        ...
        memory_metadata["hash"] = hashlib.sha256(content.encode()).hexdigest()

        # Store memory as vector with complete metadata
        await vectors.upsert_text(text=content, model=self.embedding_model, id=memory_id, metadata=memory_metadata)
        ...
        try:
            ...
            if content:
                ...
                await vectors.upsert_text(
                    text=content, model=self.embedding_model, id=memory_id, metadata=updated_metadata
                )
    ...
    async def cleanup_expired_memories(self, user_id: str, memory_type: MemoryType = MemoryType.WORKING) -> int:
        """
