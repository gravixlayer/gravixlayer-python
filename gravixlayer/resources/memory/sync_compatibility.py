...
class SyncLegacyMemoryCompatibility:
    ...
    def __init__(
        ...
        """
        Initialize synchronous memory layer - all parameters required
        ...
        """
        cloud_config = {"cloud_provider": cloud_provider, "region": region, "index_type": "serverless"}
    ...
    def search(
        ...
        """
        Search memories for a user (sync)
        ...
        """
        results = self.unified_sync_memory.search(
            query=query, user_id=user_id, memory_types=memory_types, top_k=top_k, min_relevance=min_relevance
        )

        # Convert to legacy format
        return [{"memory": result.memory, "score": result.relevance_score} for result in results]

    def get(self, memory_id: str, user_id: str) -> Optional[MemoryEntry]:
        """
        Retrieve a specific memory by ID (sync)
    ...
    def update(
        ...
        return self.unified_sync_memory.update(
            memory_id=memory_id, user_id=user_id, content=content, metadata=metadata, importance_score=importance_score
        )
    ...
    def get_memories_by_type(self, user_id: str, memory_type: MemoryType, limit: int = 50) -> List[MemoryEntry]:
        """
    ...
    def get_all_user_memories(self, user_id: str, limit: int = 100) -> List[MemoryEntry]:
        """
        Get all memories for a user (sync)
    ...
    def list_all_memories(
        self, user_id: str, limit: int = 100, sort_by: str = "created_at", ascending: bool = False
    ) -> List[MemoryEntry]:
        """
        ...
        return self.unified_sync_memory.list_all_memories(
            user_id=user_id, limit=limit, sort_by=sort_by, ascending=ascending
        )
    ...
    def cleanup_working_memory(self, user_id: str) -> int:
        ...
        for memory in working_memories:
            if memory.created_at < cutoff_time:
                if self.delete(memory.id, user_id):
                    cleaned_count += 1
...
class SyncExternalCompatibilityLayer:
    """
    Synchronous external memory layer for external APIs.
    ...
    def __init__(
        ...
        """
        Initialize synchronous external memory layer - all parameters required
    ...
    def switch_configuration(
        ...
        if embedding_model is not None and embedding_model != self.current_embedding_model:
            ...
            self.unified_sync_memory.embedding_dimension = self.unified_sync_memory._get_embedding_dimension(
                embedding_model
            )
            config_changed = True
    ...
    def list_available_indexes(self) -> List[str]:
        ...
        try:
            ...
            for idx in index_list.indexes:
                ...
                if idx.metadata and idx.metadata.get("type") in ["unified_memory_store", "gravix_memory_store"]:
    ...
    def add(
        ...
        memory_entries = self.unified_sync_memory.add(content=messages, user_id=user_id, metadata=metadata, infer=infer)
    ...
    def search(
        ...
        search_results = self.unified_sync_memory.search(
            query=query, user_id=user_id, top_k=limit, min_relevance=min_relevance
        )
    ...
    def get_all(self, user_id: str, limit: int = 100, index_name: Optional[str] = None) -> Dict[str, Any]:
        ...
        memory_results = self.unified_sync_memory.search(
            query="memory", user_id=user_id, top_k=limit, min_relevance=0.0  # Generic query  # Accept all matches
        )
    ...
    def update(self, memory_id: str, user_id: str, data: str, index_name: Optional[str] = None) -> Dict[str, str]:
        ...
        updated_memory = self.unified_sync_memory.update(memory_id=memory_id, user_id=user_id, content=data)
    ...
    def delete_all(self, user_id: str) -> Dict[str, str]:
        ...
        """
        memory_results = self.unified_sync_memory.search(
            query="memory", user_id=user_id, top_k=1000, min_relevance=0.0  # Generic query  # Large limit to get all
        )
