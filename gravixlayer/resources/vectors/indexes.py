...
class VectorIndexes:
    ...
    def list(self, page: int = 1, page_size: int = 20) -> VectorIndexList:
        """
        ...
        pagination = result.get("pagination", {"page": page, "page_size": page_size, "total": len(indexes)})

        return VectorIndexList(indexes=indexes, pagination=pagination)
    ...
    def update(
        self, index_id: str, metadata: Optional[Dict[str, Any]] = None, delete_protection: Optional[bool] = None
    ) -> VectorIndex:
        """
    ...
    def delete(self, index_id: str, force: bool = False) -> None:
        """
        ...
        if force:
            ...
            try:
                self.update(index_id, delete_protection=False)
            except Exception:
