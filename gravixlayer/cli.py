...
def parse_gpu_spec(gpu_type, gpu_count=1):
    ...
    if gpu_key not in gpu_mapping:
        raise ValueError(f"Unsupported GPU type: {gpu_type}. Supported: {list(gpu_mapping.keys())}")
...
def main():
    ...
    chat_parser.add_argument("--api-key", type=str, default=None, help="API key")
    chat_parser.add_argument("--model", required=True, help="Model name")
    chat_parser.add_argument("--system", default=None, help="System prompt (optional)")
    chat_parser.add_argument("--user", help="User prompt/message (chat mode)")
    chat_parser.add_argument("--prompt", help="Direct prompt (completions mode)")
    chat_parser.add_argument("--temperature", type=float, default=None, help="Temperature")
    chat_parser.add_argument("--max-tokens", type=int, default=None, help="Maximum tokens to generate")
    chat_parser.add_argument("--stream", action="store_true", help="Stream output")
    chat_parser.add_argument("--mode", choices=["chat", "completions"], default="chat", help="API mode")

    # Deployments parser (for deployment management)
    deployments_parser = subparsers.add_parser("deployments", help="Deployment management")
    deployments_subparsers = deployments_parser.add_subparsers(
        dest="deployments_action", help="Deployment actions", required=True
    )

    # Create deployment
    create_parser = deployments_subparsers.add_parser("create", help="Create a new deployment")
    create_parser.add_argument("--api-key", type=str, default=None, help="API key")
    create_parser.add_argument("--deployment_name", required=True, help="Deployment name")
    create_parser.add_argument("--hw_type", default="dedicated", help="Hardware type (default: dedicated)")
    create_parser.add_argument("--gpu_model", required=True, help="GPU model specification (e.g., NVIDIA_T4_16GB)")
    create_parser.add_argument("--gpu_count", type=int, default=1, help="Number of GPUs (supported values: 1, 2, 4, 8)")
    create_parser.add_argument("--min_replicas", type=int, default=1, help="Minimum replicas (default: 1)")
    create_parser.add_argument("--max_replicas", type=int, default=1, help="Maximum replicas (default: 1)")
    create_parser.add_argument("--model_name", required=True, help="Model name to deploy")
    create_parser.add_argument(
        "--auto-retry", action="store_true", help="Auto-retry with unique name if deployment name exists"
    )
    create_parser.add_argument("--wait", action="store_true", help="Wait for deployment to be ready before exiting")

    # List deployments
    list_parser = deployments_subparsers.add_parser("list", help="List all deployments")
    list_parser.add_argument("--api-key", type=str, default=None, help="API key")
    list_parser.add_argument("--json", action="store_true", help="Output as JSON")

    # Delete deployment
    delete_parser = deployments_subparsers.add_parser("delete", help="Delete a deployment")
    delete_parser.add_argument("--api-key", type=str, default=None, help="API key")
    delete_parser.add_argument("deployment_id", help="Deployment ID to delete")

    # Hardware/GPU listing
    hardware_parser = deployments_subparsers.add_parser("hardware", help="List available hardware/GPUs")
    hardware_parser.add_argument("--api-key", type=str, default=None, help="API key")
    hardware_parser.add_argument("--list", action="store_true", help="List available hardware")
    hardware_parser.add_argument("--json", action="store_true", help="Output as JSON")

    # GPU listing (alias for hardware)
    gpu_parser = deployments_subparsers.add_parser("gpu", help="List available GPUs")
    gpu_parser.add_argument("--api-key", type=str, default=None, help="API key")
    gpu_parser.add_argument("--list", action="store_true", help="List available GPUs")
    gpu_parser.add_argument("--json", action="store_true", help="Output as JSON")

    # Files parser (for file management)
    files_parser = subparsers.add_parser("files", help="File management")
    files_subparsers = files_parser.add_subparsers(dest="files_action", help="File actions", required=True)

    # Upload file
    upload_parser = files_subparsers.add_parser("upload", help="Upload a file")
    upload_parser.add_argument("--api-key", type=str, default=None, help="API key")
    upload_parser.add_argument("--file", required=True, help="Path to file to upload")
    upload_parser.add_argument("--file_name", help="Custom name for the uploaded file (optional)")
    upload_parser.add_argument(
        "--purpose",
        required=True,
        choices=["fine-tune", "assistants", "batch", "vision", "user_data", "evals"],
        help="Purpose of the file",
    )
    upload_parser.add_argument("--expires-after", type=int, help="File expiration time in seconds")

    # List files
    list_files_parser = files_subparsers.add_parser("list", help="List all files")
    list_files_parser.add_argument("--api-key", type=str, default=None, help="API key")
    list_files_parser.add_argument("--purpose", help="Filter by purpose")
    ...
    vectors_subparsers = vectors_parser.add_subparsers(dest="vectors_action", help="Vector actions", required=True)

    # Index management
    index_parser = vectors_subparsers.add_parser("index", help="Index management")
    index_subparsers = index_parser.add_subparsers(dest="index_action", help="Index actions", required=True)

    # Create index
    create_index_parser = index_subparsers.add_parser("create", help="Create a vector index")
    create_index_parser.add_argument("--api-key", type=str, default=None, help="API key")
    create_index_parser.add_argument("--name", required=True, help="Index name")
    create_index_parser.add_argument("--dimension", type=int, required=True, help="Vector dimension (1-2000)")
    create_index_parser.add_argument(
        "--metric", required=True, choices=["cosine", "euclidean", "dotproduct"], help="Similarity metric"
    )
    create_index_parser.add_argument(
        "--cloud-provider", required=True, choices=["AWS", "GCP", "Azure", "Gravix"], help="Cloud provider"
    )
    create_index_parser.add_argument("--region", required=True, help="Region ID (e.g., us-east-1, us-central1, eastus)")
    create_index_parser.add_argument(
        "--index-type", required=True, choices=["serverless", "dedicated"], help="Index type"
    )
    create_index_parser.add_argument("--vector-type", default="dense", help="Vector type (default: dense)")
    create_index_parser.add_argument("--metadata", nargs="*", help="JSON metadata for the index")
    create_index_parser.add_argument("--metadata-file", help="Path to JSON file containing metadata")
    create_index_parser.add_argument(
        "--metadata-b64", help="Base64-encoded JSON metadata (for complex JSON with spaces)"
    )
    create_index_parser.add_argument("--delete-protection", action="store_true", help="Enable delete protection")

    # List indexes
    list_indexes_parser = index_subparsers.add_parser("list", help="List all indexes")
    list_indexes_parser.add_argument("--api-key", type=str, default=None, help="API key")
    list_indexes_parser.add_argument("--json", action="store_true", help="Output as JSON")

    # Get index
    get_index_parser = index_subparsers.add_parser("get", help="Get index information")
    get_index_parser.add_argument("--api-key", type=str, default=None, help="API key")
    get_index_parser.add_argument("index_id", help="Index ID")
    get_index_parser.add_argument("--json", action="store_true", help="Output as JSON")

    # Update index
    update_index_parser = index_subparsers.add_parser("update", help="Update index")
    update_index_parser.add_argument("--api-key", type=str, default=None, help="API key")
    update_index_parser.add_argument("index_id", help="Index ID")
    update_index_parser.add_argument("--metadata", nargs="*", help="JSON metadata to update")
    update_index_parser.add_argument("--metadata-file", help="Path to JSON file containing metadata")
    ...
    # Delete index
    delete_index_parser = index_subparsers.add_parser("delete", help="Delete index")
    delete_index_parser.add_argument("--api-key", type=str, default=None, help="API key")
    delete_index_parser.add_argument("index_id", help="Index ID to delete")
    delete_index_parser.add_argument(
        "--force",
        action="store_true",
        help="Force delete by first disabling delete protection, then deleting the index",
    )

    # Vector operations
    vector_parser = vectors_subparsers.add_parser("vector", help="Vector operations")
    vector_subparsers = vector_parser.add_subparsers(dest="vector_action", help="Vector actions", required=True)

    # Upsert vector
    upsert_parser = vector_subparsers.add_parser("upsert", help="Upsert a vector")
    upsert_parser.add_argument("--api-key", type=str, default=None, help="API key")
    upsert_parser.add_argument("index_id", help="Index ID")
    upsert_parser.add_argument("--embedding", nargs="*", required=True, help="Vector embedding as JSON array")
    upsert_parser.add_argument("--id", help="Vector ID (auto-generated if not provided)")
    upsert_parser.add_argument("--metadata", nargs="*", help="JSON metadata for the vector")
    upsert_parser.add_argument("--metadata-file", help="Path to JSON file containing metadata")
    upsert_parser.add_argument("--metadata-b64", help="Base64-encoded JSON metadata")
    upsert_parser.add_argument(
        "--delete-protection", action="store_true", help="Enable delete protection for this vector"
    )

    # Upsert text vector
    upsert_text_parser = vector_subparsers.add_parser("upsert-text", help="Upsert a text vector")
    upsert_text_parser.add_argument("--api-key", type=str, default=None, help="API key")
    upsert_text_parser.add_argument("index_id", help="Index ID")
    upsert_text_parser.add_argument("--text", required=True, help="Text to convert to vector")
    upsert_text_parser.add_argument("--model", required=True, help="Embedding model name")
    upsert_text_parser.add_argument("--id", help="Vector ID (auto-generated if not provided)")
    upsert_text_parser.add_argument("--metadata", nargs="*", help="JSON metadata for the vector")
    upsert_text_parser.add_argument("--metadata-file", help="Path to JSON file containing metadata")
    upsert_text_parser.add_argument("--metadata-b64", help="Base64-encoded JSON metadata")
    upsert_text_parser.add_argument(
        "--delete-protection", action="store_true", help="Enable delete protection for this vector"
    )

    # Search vectors
    search_parser = vector_subparsers.add_parser("search", help="Search vectors")
    search_parser.add_argument("--api-key", type=str, default=None, help="API key")
    search_parser.add_argument("index_id", help="Index ID")
    search_parser.add_argument("--vector", nargs="*", required=True, help="Query vector as JSON array")
    search_parser.add_argument("--top-k", type=int, default=10, help="Number of results (default: 10)")
    search_parser.add_argument("--filter", nargs="*", help="JSON filter for metadata")
    ...
    search_parser.add_argument(
        ...
        choices=["true", "false"],
        default="false",
        help="Include vector values (true/false)",
    )

    # Search text
    search_text_parser = vector_subparsers.add_parser("search-text", help="Search using text")
    search_text_parser.add_argument("--api-key", type=str, default=None, help="API key")
    search_text_parser.add_argument("index_id", help="Index ID")
    search_text_parser.add_argument("--query", required=True, help="Search query text")
    search_text_parser.add_argument("--model", required=True, help="Embedding model name")
    search_text_parser.add_argument("--top-k", type=int, default=10, help="Number of results (default: 10)")
    search_text_parser.add_argument("--filter", nargs="*", help="JSON filter for metadata")

    # List vectors
    list_vectors_parser = vector_subparsers.add_parser("list", help="List vectors in index")
    list_vectors_parser.add_argument("--api-key", type=str, default=None, help="API key")
    list_vectors_parser.add_argument("index_id", help="Index ID")
    list_vectors_parser.add_argument("--ids-only", action="store_true", help="List only vector IDs")

    # Get vector
    get_vector_parser = vector_subparsers.add_parser("get", help="Get vector information")
    get_vector_parser.add_argument("--api-key", type=str, default=None, help="API key")
    get_vector_parser.add_argument("index_id", help="Index ID")
    get_vector_parser.add_argument("vector_id", help="Vector ID")

    # Update vector
    update_vector_parser = vector_subparsers.add_parser("update", help="Update vector metadata and delete protection")
    update_vector_parser.add_argument("--api-key", type=str, default=None, help="API key")
    update_vector_parser.add_argument("index_id", help="Index ID")
    update_vector_parser.add_argument("vector_id", help="Vector ID")
    update_vector_parser.add_argument("--metadata", nargs="*", help="JSON metadata to update")
    update_vector_parser.add_argument("--metadata-file", help="Path to JSON file containing metadata")
    update_vector_parser.add_argument("--metadata-b64", help="Base64-encoded JSON metadata")
    update_vector_parser.add_argument(
        "--delete-protection", type=str, choices=["true", "false"], help="Enable/disable delete protection (true/false)"
    )

    # Delete vector
    delete_vector_parser = vector_subparsers.add_parser("delete", help="Delete vector")
    delete_vector_parser.add_argument("--api-key", type=str, default=None, help="API key")
    ...
    batch_delete_parser = vector_subparsers.add_parser("batch-delete", help="Delete multiple vectors")
    batch_delete_parser.add_argument("--api-key", type=str, default=None, help="API key")
    batch_delete_parser.add_argument("index_id", help="Index ID")
    batch_delete_parser.add_argument("--vector-ids", required=True, help="Comma-separated list of vector IDs to delete")

    # === SANDBOX COMMAND ===
    sandbox_parser = subparsers.add_parser("sandbox", help="Sandbox management")
    sandbox_subparsers = sandbox_parser.add_subparsers(dest="sandbox_action", help="Sandbox actions", required=True)

    # Create sandbox
    create_sandbox_parser = sandbox_subparsers.add_parser("create", help="Create a new sandbox")
    create_sandbox_parser.add_argument("--api-key", type=str, default=None, help="API key")
    ...
    # List sandboxes
    list_sandboxes_parser = sandbox_subparsers.add_parser("list", help="List all sandboxes")
    list_sandboxes_parser.add_argument("--api-key", type=str, default=None, help="API key")
    list_sandboxes_parser.add_argument("--limit", type=int, default=100, help="Maximum number of results")
    list_sandboxes_parser.add_argument("--offset", type=int, default=0, help="Number of results to skip")
    list_sandboxes_parser.add_argument("--json", action="store_true", help="Output as JSON")

    # Get sandbox info
    get_sandbox_parser = sandbox_subparsers.add_parser("get", help="Get sandbox information")
    get_sandbox_parser.add_argument("--api-key", type=str, default=None, help="API key")
    get_sandbox_parser.add_argument("sandbox_id", help="Sandbox ID")
    get_sandbox_parser.add_argument("--json", action="store_true", help="Output as JSON")

    # Kill sandbox
    kill_sandbox_parser = sandbox_subparsers.add_parser("kill", help="Terminate a sandbox")
    kill_sandbox_parser.add_argument("--api-key", type=str, default=None, help="API key")
    ...
    metrics_parser.add_argument("--api-key", type=str, default=None, help="API key")
    ...
    # Get host URL
    host_url_parser = sandbox_subparsers.add_parser("host", help="Get sandbox host URL")
    host_url_parser.add_argument("--api-key", type=str, default=None, help="API key")
    host_url_parser.add_argument("sandbox_id", help="Sandbox ID")
    host_url_parser.add_argument("port", type=int, help="Port number")

    # Run command
    run_command_parser = sandbox_subparsers.add_parser("run", help="Execute a command in sandbox")
    run_command_parser.add_argument("--api-key", type=str, default=None, help="API key")
    ...
    run_command_parser.add_argument("--working-dir", help="Working directory")
    run_command_parser.add_argument("--timeout", type=int, help="Timeout in milliseconds")

    # Run code
    run_code_parser = sandbox_subparsers.add_parser("code", help="Execute code in sandbox")
    run_code_parser.add_argument("--api-key", type=str, default=None, help="API key")
    run_code_parser.add_argument("sandbox_id", help="Sandbox ID")
    run_code_parser.add_argument("code", help="Code to execute")
    run_code_parser.add_argument("--language", default="python", help="Programming language")
    run_code_parser.add_argument("--context-id", help="Code execution context ID")

    # Code context management
    context_parser = sandbox_subparsers.add_parser("context", help="Code context management")
    context_subparsers = context_parser.add_subparsers(dest="context_action", help="Context actions", required=True)

    # Create context
    create_context_parser = context_subparsers.add_parser("create", help="Create code context")
    create_context_parser.add_argument("--api-key", type=str, default=None, help="API key")
    create_context_parser.add_argument("sandbox_id", help="Sandbox ID")
    create_context_parser.add_argument("--language", default="python", help="Programming language")
    create_context_parser.add_argument("--cwd", help="Working directory")

    # Get context
    get_context_parser = context_subparsers.add_parser("get", help="Get code context info")
    get_context_parser.add_argument("--api-key", type=str, default=None, help="API key")
    get_context_parser.add_argument("sandbox_id", help="Sandbox ID")
    get_context_parser.add_argument("context_id", help="Context ID")

    # Delete context
    delete_context_parser = context_subparsers.add_parser("delete", help="Delete code context")
    delete_context_parser.add_argument("--api-key", type=str, default=None, help="API key")
    ...
    # File operations
    file_parser = sandbox_subparsers.add_parser("file", help="File operations")
    file_subparsers = file_parser.add_subparsers(dest="file_action", help="File actions", required=True)

    # Read file
    read_file_parser = file_subparsers.add_parser("read", help="Read file from sandbox")
    read_file_parser.add_argument("--api-key", type=str, default=None, help="API key")
    read_file_parser.add_argument("sandbox_id", help="Sandbox ID")
    read_file_parser.add_argument("path", help="File path")

    # Write file
    write_file_parser = file_subparsers.add_parser("write", help="Write file to sandbox")
    write_file_parser.add_argument("--api-key", type=str, default=None, help="API key")
    ...
    # List files
    list_files_parser = file_subparsers.add_parser("list", help="List files in sandbox")
    list_files_parser.add_argument("--api-key", type=str, default=None, help="API key")
    list_files_parser.add_argument("sandbox_id", help="Sandbox ID")
    list_files_parser.add_argument("path", help="Directory path")

    # Delete file
    delete_file_parser = file_subparsers.add_parser("delete", help="Delete file from sandbox")
    delete_file_parser.add_argument("--api-key", type=str, default=None, help="API key")
    delete_file_parser.add_argument("sandbox_id", help="Sandbox ID")
    delete_file_parser.add_argument("path", help="File path")

    # Make directory
    mkdir_parser = file_subparsers.add_parser("mkdir", help="Create directory in sandbox")
    mkdir_parser.add_argument("--api-key", type=str, default=None, help="API key")
    mkdir_parser.add_argument("sandbox_id", help="Sandbox ID")
    mkdir_parser.add_argument("path", help="Directory path")

    # Upload file
    upload_parser = file_subparsers.add_parser("upload", help="Upload file to sandbox")
    upload_parser.add_argument("--api-key", type=str, default=None, help="API key")
    ...
    # Download file
    download_parser = file_subparsers.add_parser("download", help="Download file from sandbox")
    download_parser.add_argument("--api-key", type=str, default=None, help="API key")
    ...
    # Template management
    template_parser = sandbox_subparsers.add_parser("template", help="Template management")
    template_subparsers = template_parser.add_subparsers(dest="template_action", help="Template actions", required=True)

    # List templates
    list_templates_parser = template_subparsers.add_parser("list", help="List available templates")
    list_templates_parser.add_argument("--api-key", type=str, default=None, help="API key")
    list_templates_parser.add_argument("--limit", type=int, default=100, help="Maximum number of results")
    list_templates_parser.add_argument("--offset", type=int, default=0, help="Number of results to skip")
...
def handle_deployments_commands(args):
    ...
    try:
        if args.deployments_action == "create":
                                ...
                                if deployment_created:
                                    ...
                                    if hasattr(args, "wait") and args.wait:
                                        wait_for_deployment_ready(
                                            client, created_deployment.deployment_id, created_deployment.deployment_name
                                        )
                                    else:
                                    ...
                                    if genuine_duplicate:
                                        print(
                                            f"ERROR: Deployment creation failed: deployment with name '{original_name}' already exists."
                                        )
                                        if hasattr(args, "auto_retry") and args.auto_retry:
                                            print("Auto-retry was already attempted but failed.")
                                        else:
                                            print(
                                                f'Try with --auto-retry flag: gravixlayer deployments create --deployment_name "{original_name}" --gpu_model "{args.gpu_model}" --model_name "{args.model_name}" --auto-retry'
                                            )
                                    else:
            ...
            if args.json:
                print(json.dumps([d.model_dump() for d in deployments], indent=2))
            else:
                if not deployments:
            ...
            if hasattr(args, "list") and args.list:
                ...
                if hasattr(args, "json") and getattr(args, "json", False):
                    ...
                    if not accelerators:
                        ...
                        print(
                            f"Available {'Hardware' if args.deployments_action == 'hardware' else 'GPUs'} ({len(accelerators)} found):"
                        )
                        print()
                ...
                print(
                    f"Use --list flag to list available {'hardware' if args.deployments_action == 'hardware' else 'GPUs'}"
                )
        ...
        if args.mode == "chat":
            ...
            if args.stream:
                ...
                completion = client.chat.completions.create(
                    model=args.model, messages=messages, temperature=args.temperature, max_tokens=args.max_tokens or 150
                )
    ...
    try:
        ...
        if args.files_action == "upload":
            ...
            with open(args.file, "rb") as f:
                upload_args = {"file": f, "purpose": args.purpose}
            ...
            if args.vector_action == "upsert":
                ...
                try:
                    ...
                    if actual_dim != expected_dim:
                        ...
                        print(
                            f"   Tip: Use 'gravixlayer vectors vector upsert-text' for automatic embedding generation"
                        )
                ...
                results = vectors.search_text(
                    query=args.query, model=args.model, top_k=getattr(args, "top_k", 10), filter=filter_dict
                )
                ...
                if metadata is None and delete_protection is None:
                    print(
                        "ERROR: At least one field must be provided for update (--metadata, --metadata-file, --metadata-b64, or --delete-protection)"
                    )
                    return

                try:
                    updated_vector = vectors.update(
                        vector_id=args.vector_id, metadata=metadata, delete_protection=delete_protection
                    )
...
def handle_template_commands(args, client):
    ...
    try:
        if args.template_action == "list":
            templates = client.sandbox.templates.list(limit=args.limit, offset=args.offset)
            ...
            if args.json:
                ...
                for template in templates.templates:
                    ...
                    print(
                        f"   Resources: {template.vcpu_count} vCPU, {template.memory_mb}MB RAM, {template.disk_size_mb}MB disk"
                    )
