...
# ===================================================================
# Sync Client — warmup (cold TLS / connection pool)
# ===================================================================


class TestSyncWarmup:
    def test_warmup_succeeds_on_200(self, mock_api):
    ...
    def test_warmup_401_raises(self, mock_api):
        mock_api.get(f"{AGENTS_BASE}/runtime").mock(return_value=httpx.Response(401))
        c = GravixLayer(api_key=TEST_API_KEY, base_url=TEST_BASE_URL)
    ...
    def test_warmup_on_init_calls_list_endpoint(self, mock_api):
        ...
        c = GravixLayer(api_key=TEST_API_KEY, base_url=TEST_BASE_URL, warmup_on_init=True)
        c.close()
        assert route.called
...
class TestAsyncWarmup:
    @pytest.mark.asyncio
    async def test_warmup_succeeds_on_200(self, mock_api):
        mock_api.get(f"{AGENTS_BASE}/runtime").mock(
            return_value=httpx.Response(200, json={"runtimes": [], "total": 0})
        )
        c = AsyncGravixLayer(api_key=TEST_API_KEY, base_url=TEST_BASE_URL)
