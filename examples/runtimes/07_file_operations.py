...
read_result = sandbox.file.read("/workspace/hello.txt")
print(f"Read       : {read_result.content.strip()}")

# ---------------------------------------------------------------------------
# 2. Create a directory (recursive by default)
# ---------------------------------------------------------------------------
sandbox.file.create_directory("/workspace/project/src")
...
"""
sandbox.file.write("/workspace/project/src/main.py", script)
print("Wrote      : /workspace/project/src/main.py")

# ---------------------------------------------------------------------------
# 4. List a directory
# ---------------------------------------------------------------------------
file_list = sandbox.file.list("/workspace/project/src")
...
batch_result = sandbox.file.write_many(entries)
...
info_run = sandbox.file.get_info(STAT_PATH)
...
perm_resp = sandbox.file.set_permissions(STAT_PATH, "600")
print(f"chmod      : {STAT_PATH} -> {perm_resp.message!r} ok={perm_resp.success}")

info_after = sandbox.file.get_info(STAT_PATH)
...
# 9. List project tree
# ---------------------------------------------------------------------------
file_list = sandbox.file.list("/workspace/project")
...
# ---------------------------------------------------------------------------
up = sandbox.file.upload_file(
    BytesIO(b"uploaded from laptop\n"), path="/workspace/from_local.txt"
)
print(f"\nLocal file : uploaded to {up.path!r} ({up.message})")

# ---------------------------------------------------------------------------
# 11. Download bytes from the sandbox
# ---------------------------------------------------------------------------
downloaded = sandbox.file.download_file("/workspace/hello.txt")
...
# ---------------------------------------------------------------------------
owner = sandbox.run_cmd(command="id", args=["-un"]).stdout.strip()
chowned = sandbox.file.chown("/workspace/project/src-copy", user=owner, recursive=True)
print(f"chown      : {chowned.path} -> {owner} ok={chowned.success}")

# ---------------------------------------------------------------------------
# 15. Find files by name (glob) and by content
# ---------------------------------------------------------------------------
by_name = sandbox.file.find("/workspace/project", glob="*.py")
...
by_content = sandbox.file.find("/workspace/project", pattern="GravixLayer")
...
scoped = sandbox.file.find("/workspace/project", pattern="version", glob="*.py")
...
print(f"verify     : {sandbox.file.read('/workspace/project/src/main.py').content.strip()}")
...
def make_changes() -> None:
    ...
    sandbox.file.write("/workspace/project/watched.txt", "first")
    time.sleep(0.5)
    sandbox.file.write("/workspace/project/watched.txt", "second")
    time.sleep(0.5)
    sandbox.file.delete("/workspace/project/watched.txt")
...
seen = 0
for event in sandbox.file.watch("/workspace/project", recursive=True):
...
# Let it finish before the sandbox goes away.
changes.join(timeout=30)

# ---------------------------------------------------------------------------
# 18. Delete a file
# ---------------------------------------------------------------------------
sandbox.file.delete("/workspace/hello.txt")
